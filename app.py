...
st.markdown(
    ...
    .metric-card p  { font-size: 0.82rem; color: #94a3b8; margin: 0.3rem 0 0; }
    ...
    .chip-green  { background: rgba( 52,211,153,0.18); color: #34d399; border: 1px solid rgba( 52,211,153,0.35); }
    .chip-yellow { background: rgba(251,191, 36,0.18); color: #fbbf24; border: 1px solid rgba(251,191, 36,0.35); }
    .chip-red    { background: rgba(248, 113,113,0.18); color: #f87171; border: 1px solid rgba(248,113,113,0.35); }
    ...
    /* ── Progress bar ── */
    .stProgress > div > div > div { background: linear-gradient(90deg, #6366f1, #ec4899); }
    ...
    thead tr th { background: rgba(99,102,241,0.20) !important; }
...
with st.sidebar:
    ...
    st.caption(
        ...
        "Ranks candidates using semantic similarity + skill matching."
    )
...
st.markdown(
    ...
    <div style='text-align:center; padding: 1.5rem 0 0.5rem;'>
        ...
        <p style='color:#94a3b8; font-size:1rem; margin:0;'>
            Upload resumes · Paste a job description · Rank candidates instantly
        </p>
    </div>
    <hr style='border:none; border-top:1px solid rgba(255,255,255,0.08); margin:1rem 0 1.5rem;'>
    """,
    unsafe_allow_html=True,
)
...
with col_upload:
    ...
    if uploaded_files:
        ...
        for f in uploaded_files:
            ...
            st.markdown(
                f"<small style='color:#94a3b8;'>📄 {f.name} &nbsp;·&nbsp; {size_kb} KB</small>",
                unsafe_allow_html=True,
            )
...
def _validate_inputs() -> bool:
    ...
    if not uploaded_files:
        st.error("⚠️ Please upload at least one PDF resume before analyzing.")
        return False
    if not job_description or not job_description.strip():
        st.error("⚠️ Job description cannot be empty. Please paste a job description.")
        return False
...
def _run_pipeline(files, jd_text: str, threshold: float) -> None:
    ...
    Steps:
        ...
        2. Clean & preprocess text (preprocess)
    ...
    # ── Step 1 & 2 & 3: Per-resume extraction + preprocessing + skills ──────
    ...
    for i, uploaded_file in enumerate(files):
        ...
        try:
            ...
            if not raw.strip():
                st.warning(
                    f"⚠️ **{uploaded_file.name}** — No text could be extracted. "
            ...
            st.warning(f"⚠️ **{uploaded_file.name}** — Failed to process: {exc}")
            error_files.append(uploaded_file.name)
...
if st.session_state["processed"] and st.session_state["ranked_df"] is not None:
    ...
    if st.session_state["error_files"]:
        st.markdown("---")
        st.markdown("**⚠️ Files that could not be processed:**")
